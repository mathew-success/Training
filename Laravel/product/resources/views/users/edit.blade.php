    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-md-5">
                <div class="row">
                    <div class="col-sm-10">
                        <h3 class="text-left">Update User Information</h3>
                    </div>
                    <div class="col-sm-2">
                ...
                <form action="{{ route('user.update',$user->id) }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            ...
                            <input type="text" name="phone_no" value="{{ $user->phone_no }}" class="form-control" id="">
                        </div>
                        <div class="col-sm-6">
                            <label>Role : </label>
                            <select name="role_id" class="form-control">
                                @foreach($roles as $role)
                                <option value="{{ $role->id }}" {{ $role->id === $actualRole ? 'selected':''}}>{{ $role->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
